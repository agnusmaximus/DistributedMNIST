...
class MNISTData(Dataset):
  ...
  def __init__(self, subset):
    super(MNISTData, self).__init__('ImageNet', subset)
  ...
  def data_files(self):
    if os.path.exists(FLAGS.data_dir) and os.path.isdir(FLAGS.data_dir):
      super(MNISTData, self).data_files()
    else:
      print("Data path does not exist: %s" % FLAGS.data_dir)
      exit(-1)

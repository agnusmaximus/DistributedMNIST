...
# replica.
class SyncReplicasOptimizerModified(optimizer.Optimizer):
  ...
  * `sync_rep_local_step`, one per replica. Compared against the global_step in
  ...
  opt = tf.SyncReplicasOptimizerModified(opt, replicas_to_aggregate=50,
  ...
  To use SyncReplicasOptimizerModified with an `Estimator`, you need to send
  ...
  def apply_gradients(self, grads_and_vars, worker_id, global_step=None, name=None):
    ...
    with ops.name_scope(None, self._name):
      ...
      with ops.device(global_step.device), ops.name_scope(""):
        ...
        with tf.device('/job:worker/task:%d' % worker_id):
          dbg_print_2 = tf.Print(global_step, [global_step], message="Start update op")

        with tf.device('/job:worker/task:%d' % worker_id):
          dbg_print_3 = tf.Print(global_step, [global_step], message="Done update op")

        with tf.device('/job:worker/task:%d' % worker_id):
          dbg_print_4 = tf.Print(global_step, [global_step], message="Done enqueueing op")
        ...
        with ops.control_dependencies([dbg_print_2]):
          with ops.control_dependencies([update_op]):
            with ops.control_dependencies([dbg_print_3]):
              ...
              with ops.control_dependencies([print_tokens_op]):
  ...
  def make_session_run_hook(self, is_chief, num_tokens=-1):
    ...
    return _SyncReplicasOptimizerModifiedHook(self, is_chief, num_tokens)


class _SyncReplicasOptimizerModifiedHook(session_run_hook.SessionRunHook):
  """A SessionRunHook handles ops related to SyncReplicasOptimizerModified."""

  def __init__(self, sync_optimizer, is_chief, num_tokens):
    """Creates hook to handle SyncReplicaOptimizer initialization ops.

    Args:
      sync_optimizer: `SyncReplicasOptimizerModified` which this hook will initialize.
  ...
  def begin(self):
    if self._sync_optimizer._gradients_applied is False:  # pylint: disable=protected-access
      raise ValueError(
          "SyncReplicasOptimizerModified.apply_gradient should be called before using "
          "the hook.")
    if self._is_chief:
  ...
  def after_create_session(self, session, coord):
    """Runs SyncReplicasOptimizerModified initialization ops."""
    local_init_success, msg = session_manager._ready(  # pylint: disable=protected-access
        self._ready_for_local_init_op, session,
        "Model is not ready for SyncReplicasOptimizerModified local init.")
    if not local_init_success:
      raise RuntimeError(
          "Init operations did not make model ready for SyncReplicasOptimizerModified "

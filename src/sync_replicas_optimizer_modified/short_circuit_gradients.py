...
#####################################################################
        ...
        def zero_grad_function(prefetch_vars):
            zero_grads = []
            #with ops.name_scope(op.name + "_grad"):
              # pylint: disable=protected-access
              #with ops.get_default_graph()._original_op(op):
        ...
        def in_grad_function(prefetch_vars):
            with ops.name_scope(op.name + "_grad"):
              with ops.get_default_graph()._original_op(op):
                if grad_fn:
                  # If grad_fn was found, do not use SymbolicGradient even for
        ...
        if not none_gradient:
          with ops.control_dependencies(out_grads):
            ...
            for i, inp in enumerate(op.inputs):
              ...
              tf.logging.info("YO %d %d " % (dat_transfer.op._id, inp.op._id))
              dat_transfer.op._id_value = inp.op._id
              tf.logging.info("AFTER %d %d " % (dat_transfer.op._id, inp.op._id))

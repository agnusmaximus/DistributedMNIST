  ...
  * `sync_rep_local_step`, one per replica. Compared against the global_step in
  ...
  def apply_gradients(self, grads_and_vars, worker_id, global_step=None, name=None):
    ...
    with ops.name_scope(None, self._name):
      ...
      finished_phase_1 = []
      for i in range(self._total_num_replicas):
        dequeue = tf.while_loop(lambda x : tf.less(self._p1_finished_queues[i].dequeue(), global_step.ref()),
                                lambda x : x,
                                [global_step])
      ...
      with ops.device(global_step.device), ops.name_scope(""):
        ...
        with ops.control_dependencies([update_op]):
          with ops.control_dependencies([logging_ops.Print(global_step, [global_step], message="QueueRunner enqueueing to start next iteration (global step)...")]):
            ...
            for worker in range(self._total_num_replicas):
              with ops.control_dependencies([tf.Assert(tf.equal(self._sync_token_queues[worker].size(), 0), self._sync_token_queues[worker].size())]):
